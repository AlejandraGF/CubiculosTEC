...
function GestEstudiantes(props) {
    const navigationN = useNavigation();

    //Mauricio S
    const [data, setData] = useState([])
    const [loading, setLoading] = useState(true)
    const url = "http://192.168.18.73:3000/estudiantes";
                ...
                <View style={MainScreenStyles.cubContainer}>
                    {
                        loading ? (<Text> Cargando... </Text>) : (
                            data.map((post) => (
                                <View key={post.idEstudiante}>
                                    <Text style={{ fontSize: 20, fontWeight: 'bold' }}>{post.idEstudiante}</Text>
                                    <Text style={{ fontSize: 20 }}>ID: {post.nombre}</Text>
...
export default GestEstudiantes;
